...
public:
	...
	static void Administer(std::vector<Cluster<person_type> >* clusters, std::vector<double>& immunity_distribution,
		...
		// initiate a vector to count the population per age class [0-100]
			...
			// note: focusing on measles, we expect the number of susceptible individuals to be less compared to the number
			// of immune.
			...
			while (total_num_susceptible > 0) {
				...
				for (unsigned int i_p = 0; i_p < household_size && total_num_susceptible > 0; i_p++) {
					...
					// if individual is immune and his/her age class has not reached the quota => make susceptible
	...
	static void AdministerCocoon(std::vector<Cluster<person_type> >* clusters, double immunity_rate, double adult_age_min,
	...
	static void Apply(const std::string s, std::shared_ptr<Simulator<global_information_policy, local_information_policy, belief_policy, behaviour_policy>> sim,
