...
public:
	...
	static void Apply(const std::string s,
			  ...
			  util::Random& rng)
	{
		const std::string immunity_profile =
		    pt_config.get<string>("run." + StringUtils::ToLower(s) + "_profile");
		...
		if (immunity_profile == "None") {
			...
			if (immunity_profile == "Cocoon") {
				...
				AdministerCocoon(immunity_clusters, immunity_rate, adult_age_min, adult_age_max,
						 child_age_min, child_age_max, rng);
			} else {
				if (immunity_profile == "Random") {
					...
					immunity_distribution =
					    PtreeUtils::GetDistribution(pt_disease, xml_immunity_profile);
				}
